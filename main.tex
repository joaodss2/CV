...
\begin{itemize}
  \item Lead the DevOps Engineering and Developer Experience stream, defining standards, automation patterns, and golden paths to accelerate delivery.
  \item Evaluate, test, and recommend development tooling (build, packaging, debugging, local env, quality) to reduce feedback cycles and improve developer throughput.
  \item Architect and drive migration from Docker workloads on standalone EC2 instances to Kubernetes, standardizing deployments and improving scalability and reliability.
  \item Manage AWS cloud platform resources (networking, IAM, security, cost governance) and multi-environment single-tenant infrastructure (dev, QA, staging, production).
  \item Administer Kubernetes clusters (provisioning, upgrades, autoscaling, RBAC, quotas, secrets management) ensuring secure, efficient runtime operations.
  \item Build and maintain the observability stack (Prometheus, Grafana, Loki), implementing dashboards, alerting rules, and reliability practices to support proactive monitoring.
  \item Implement and enforce a resilient, two-eyes gated release process in CI/CD (GitHub) with approval workflows, automated validation, and production protection measures.
  \item Govern access management for developers and QA across AWS, Kubernetes, and CI/CD, applying least-privilege principles and auditable role-based controls.
